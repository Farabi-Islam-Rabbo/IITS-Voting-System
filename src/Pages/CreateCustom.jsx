...
function CreateCustom({ user }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    const response = await CreateCustoms(FormData);
    console.log(response);
    const { data, message, status } = response;
    if (status) {
      toast("Custom Created!", {
        type: "success",
      });
      setLoading(false);
      navigate(`/customs`);
  ...
  return (
    <MainWrapper>
      <Sidebar title="Create Custom" breadcrumb={breadcrumbs}>
...
export default connect(mapStateToProps)(CreateCustom);
