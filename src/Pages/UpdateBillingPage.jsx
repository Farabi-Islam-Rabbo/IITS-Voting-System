...
function UpdateBillingPage({ user }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    const response = await UpdateBilling(id, FormData);
  ...
  const setData = async (response) => {
    setAmountFrom(response.data?.amountFrom);
    setAmountTo(response.data?.amountTo);
    setChargeType(
      chargeTypes.find((x) => x.value == response.data?.chargeType)
    );
  ...
  return (
    <MainWrapper>
      <Sidebar title="Update Billing" breadcrumb={breadcrumbs}>
...
export default connect(mapStateToProps)(UpdateBillingPage);
